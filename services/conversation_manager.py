...
class ConversationManager:
    ...
    @staticmethod
    def update_conversation_backend_type(conversation_id, backend_type):
        """Update conversation backend type."""
        db = get_db()
        db.execute(
            'UPDATE conversations SET backend_type = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
            (backend_type, conversation_id)
        )
        db.commit()
