...
let isTutoringMode = true; // Default to tutoring mode
let tutoringCategories = {};
...
function enableChatbox() {
    ...
    if (inputContainer && messageInput && sendBtn) {
        ...
        messageInput.placeholder = isTutoringMode ? 'Ask me anything you want to learn...' : 'Type your message...';
        sendBtn.disabled = false;
...
async function init() {
    ...
    try {
        ...
        // Initialize tutoring mode
        console.log('Initializing tutoring mode...');
...
async function createNewChat() {
    ...
    if (isTutoringMode) {
        // Reset current conversation and show tutoring welcome
        currentConversationId = null;
        updateWelcomeScreen();
        return;
    }
...
async function loadConversation(conversationId) {
    try {
        ...
        // Handle tutoring conversations - they use regular backends, don't need switching
        const isOldTutoringConversation = conversationBackend === 'tutoring_llamacpp';
        const actualBackend = isOldTutoringConversation ? 'llamacpp' : conversationBackend;
        
        if (actualBackend !== currentActiveBackend && !isOldTutoringConversation) {
            console.log(`Conversation created with ${actualBackend}, switching from ${currentActiveBackend}`);
            
            // Try to switch to the conversation's backend
            try {
                await selectBackend(actualBackend);
                ...
                showBackendSwitchNotification(
                    `⚠️ Conversation was created with ${actualBackend} backend, but switching failed. Using ${currentActiveBackend} instead.`
                );
            }
        } else if (isOldTutoringConversation) {
            console.log('Loading old tutoring conversation - using llamacpp backend');
        }
...
async function sendMessage() {
    ...
    // Handle tutoring mode
    if (isTutoringMode) {
        sendTutoringMessage(message);
        return;
    }
    
    // Check if conversation is selected (only for normal mode)
...
// Tutoring Mode Functions
async function switchToTutoringMode() {
    console.log('switchToTutoringMode called');
    isTutoringMode = true;
    updateModeToggle();
    updateWelcomeScreen();
    await loadTutoringCategories();
    
    // Update placeholder text
    const messageInput = document.getElementById('messageInput');
    if (messageInput) {
        messageInput.placeholder = 'Ask me anything you want to learn...';
    }
    
    // Save mode preference
    localStorage.setItem('chatollama_mode', 'tutoring');
    
    console.log('Switched to tutoring mode');
}

function switchToNormalMode() {
    isTutoringMode = false;
    updateModeToggle();
    updateWelcomeScreen();
    ...
    // Save mode preference
    localStorage.setItem('chatollama_mode', 'normal');
...
function updateModeToggle() {
    ...
    const tutoringBtn = document.getElementById('tutoringModeBtn');
    
    if (normalBtn && tutoringBtn) {
        if (isTutoringMode) {
            normalBtn.classList.remove('active');
            tutoringBtn.classList.add('active');
        } else {
            normalBtn.classList.add('active');
            tutoringBtn.classList.remove('active');
        }
    }
}

function updateWelcomeScreen() {
    console.log('updateWelcomeScreen called, isTutoringMode:', isTutoringMode, 'currentConversationId:', currentConversationId);
    const noConversation = document.getElementById('noConversation');
    const tutoringWelcome = document.getElementById('tutoringWelcome');
    
    console.log('noConversation element:', noConversation);
    console.log('tutoringWelcome element:', tutoringWelcome);
    
    // Don't update welcome screen if we have an active conversation
    if (currentConversationId) {
        console.log('Active conversation exists, skipping welcome screen update');
        return;
    }
    
    if (noConversation && tutoringWelcome) {
        const inputContainer = document.getElementById('inputContainer');
        if (isTutoringMode) {
            console.log('Showing tutoring welcome screen');
            noConversation.style.display = 'none';
            tutoringWelcome.style.display = 'block';
            if (inputContainer) inputContainer.style.display = 'none';
        } else {
            console.log('Showing normal welcome screen');
            noConversation.style.display = 'block';
            tutoringWelcome.style.display = 'none';
            if (inputContainer) inputContainer.style.display = 'none';
        }
    } else {
        console.log('Missing welcome screen elements');
    }
}

async function loadTutoringCategories() {
    console.log('loadTutoringCategories called');
    try {
        const response = await fetch('/api/tutoring/categories');
        console.log('Categories response status:', response.status);
        if (response.ok) {
            const data = await response.json();
            console.log('Categories data:', data);
            tutoringCategories = data.categories;
        } else {
            console.error('Failed to load categories:', response.status);
        }
    } catch (error) {
        console.error('Error loading tutoring categories:', error);
    }
}

function startTutoringCategory(category) {
    console.log('Starting tutoring category:', category);
    const categoryData = tutoringCategories[category];
    if (!categoryData) {
        console.log('No category data found for:', category);
        // Continue anyway with a generic message
    }
    
    // Create a starter message based on the category
    let starterMessage = '';
    switch (category) {
        case 'GUIDANCE':
            starterMessage = "I need some study guidance and learning strategies.";
            break;
        case 'NEW_TOPIC':
            starterMessage = "I want to learn about a new topic. Can you help me explore it step by step?";
            break;
        case 'HOMEWORK_HELP':
            starterMessage = "I need help with my homework. Can you guide me through it?";
            break;
        case 'ASSESSMENT':
            starterMessage = "I'd like to test my understanding of a topic. Can you assess my knowledge?";
            break;
        default:
            starterMessage = "I'm ready to learn!";
    }
    
    console.log('Creating tutoring chat with message:', starterMessage);
    // Create new tutoring conversation
    createTutoringChat(starterMessage);
}

async function createTutoringChat(initialMessage = '') {
    console.log('createTutoringChat called with message:', initialMessage);
    try {
        ...
        const response = await fetch('/api/tutoring/chat', {
            ...
            body: JSON.stringify({
                message: initialMessage || "I'm ready to start learning!",
                model: selectedModel
            })
        });

        console.log('Tutoring chat response status:', response.status);
        
        if (response.ok) {
            const data = await response.json();
            console.log('Tutoring chat response data:', data);
            currentConversationId = data.conversation_id;
            
            // Load the conversation
            await loadConversation(currentConversationId);
            
            // Update UI - don't call updateWelcomeScreen since we have a conversation now
            enableChatbox();
            
            // Focus on input if no initial message
            if (!initialMessage) {
                const messageInput = document.getElementById('messageInput');
                if (messageInput) {
                    messageInput.focus();
            ...
            const errorData = await response.json();
            console.error('Failed to create tutoring chat:', errorData);
        }
    } catch (error) {
        console.error('Error creating tutoring chat:', error);
    }
}
...
async function sendTutoringMessage(message) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Add tutoring response with category indicator
            const responseContent = data.response;
            const category = data.category;
            
            // Parse tutoring response (look for follow-up questions)
            const parts = responseContent.split('\n\n💡 ');
            const mainResponse = parts[0];
            const followUp = parts[1] ? `💡 ${parts[1]}` : '';
            
            // Add category badge and main response
            let displayContent = `<div class="tutoring-category-badge">${category}</div>\n${mainResponse}`;
            
            // Add follow-up question if present
            if (followUp) {
                displayContent += `\n\n<div class="tutoring-followup">${followUp}</div>`;
            }
            ...
            const errorData = await response.json();
            addMessageToChat('assistant', `I apologize, but I encountered an error: ${errorData.error}. Let me help you learn step by step instead. What would you like to explore?`);
        }
    } catch (error) {
        console.error('Error sending tutoring message:', error);
        addMessageToChat('assistant', 'I apologize for the technical difficulty. As your tutor, I\'m here to help you learn. What topic would you like to explore together?');
    } finally {
        isLoading = false;
...
async function loadModePreference() {
    console.log('loadModePreference called');
    const savedMode = localStorage.getItem('chatollama_mode');
    console.log('Saved mode:', savedMode);
    if (savedMode === 'normal') {
        switchToNormalMode();
    } else {
        await switchToTutoringMode(); // Default to tutoring mode
    }
}
